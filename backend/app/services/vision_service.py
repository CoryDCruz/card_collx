...
"""
    ...
    async def extract_card_metadata_from_bytes(
        self,
        image_bytes: bytes
    ) -> Tuple[Optional[Dict], Optional[str], Optional[str]]:
        """
        Extract card metadata from in-memory image bytes using GPT-4 Vision.

        Args:
            image_bytes: Raw image bytes (already processed)
